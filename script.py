class Pokemon:
  """ To create a pokemon, give it a name, type, and level. Its max health is equal to its level. Its starting health is equal to the max health. Also, the pokemon is not knocked out."""
  ...
  def __repr__(self):
    ...
    return f"This level {self.level} {self.name} has {self.health} hit points remaining. It's a {self.type} type Pokemon."

  def lose_health(self, damage):
    # Decrease health from a pokemon and prints the current health remaining.
    self.health -= damage
    if self.health > 0:
      #If pokemon has health greater than 0 after being damaged, then print out current health.
      self.health - damage
      print(f"{self.name} now has {self.health} health remaining.")
    else:
      #If pokemon health becomes 0 or less, then set health to 0 and knock out the pokemon. 
      self.health = 0
      self.knocked_out()
    
  def gain_health(self, amount):
    # Increase a pokemon's health when a potion is used. Then print the current health remaining.
    ...
    if self.health == 0:
      ...
      self.health += amount
      # Make sure the health does not go over the max health. Health gained is equal to the difference between max health and health before the use of the potion.
      if self.health > self.max_health:
        ...
        # If health increased is below max health, then health gained is equal to the value of the potion. 
  ...
  def knocked_out(self):
    ...
    # A knocked out pokemon should not have any health. We check to see if the knocked out pokemon health is set to 0. If its not, then we set it to zero. This is just a safety precaution. 
    if self.health != 0:
      self.health = 0
    print(f"{self.name} was knocked out!")

  def revive(self):   
    # If the pokemon health is 0 and the player uses the revival potion, then increase the pokemon's health by half of its max health. Also, change the status of the pokemon "knocked out" to False. 
    if self.health == 0:
  ...
  def attack(self, other_pokemon):
    # Pokemon attacks and deals damage to another pokemon. The other pokemon loses health.
    damage = 0
    # Check to make sure the pokemon isn't knocked out.
    if self.is_knocked_out == True:
      print(f"{self.name} can't attack because it is knocked out!")
    # If the attacking pokemon has an advantage over the other pokemon, then it deals damage equal to twice the attacking pokemon's level.
      ...
      # If the attacking pokemon has a disadvantange, then it deals damage equal to half the attacking pokemon level.
    elif (self.type == "Grass" and other_pokemon.type == "Fire") or (self.type == "Fire" and other_pokemon.type == "Water") or (self.type == "Water" and other_pokemon.type == "Grass"):
      damage += round(0.5 * self.level)      
      print(f"{self.name} attacked {other_pokemon.name} for {damage} damage.")
      print("It's not very effective...")
      other_pokemon.lose_health(damage)
    # If the attacking pokemon has neither advantange or disadvantage, then it deals damage equal to its level to the other pokemon. 
    else:
      damage += self.level
      print(f"{self.name} attacked {other_pokemon.name} for {damage} damage.")
      other_pokemon.lose_health(damage)
...
class Trainer:
  """ To create a trainer, give it a list of pokemon, a number of potions on hand, and a name. Also, when the trainer is created, the first pokemon in their list of pokemon is considered an active pokemon which is the index 0 in the list."""
  ...
  def use_potion(self):
    # The trainer uses a potion on the active pokemon, assuming the trainer has at least one potion on hand. 
    if self.potions > 0:
      self.potions -= 1
  ...
  def use_revival_potion(self):
    # The trainer uses a revival potion on a knocked out pokemon, assuming the trainer has at least one potion on hand.
    if self.revival_potions > 0:
      self.revival_potions -= 1
      print(f"You used a revival potion on {self.pokemons[self.current_pokemon].name}.")
...
# Six pokemons are made with different levels. If no level is given, its default level is 5.
a = Charmander(7)
b = Squirtle()
c = Squirtle(1)
d = Bulbasaur(10)
...
print()
print(trainer_two.potions) # Testing to see if the number of potions remaining on hand also decreased. 
print()
trainer_one.attack_other_trainer(trainer_two)
print()
...
#Testing pokemon cannot health past its maximum health, a pokemon cannot attack if its knocked out, cannot switch to a pokemon that is knocked out
trainer_two.switch_active_pokemon(0)
trainer_one.attack_other_trainer(trainer_two)
trainer_one.attack_other_trainer(trainer_two)
trainer_one.attack_other_trainer(trainer_two)
trainer_two.attack_other_trainer(trainer_one) # Knocked out Pokemon cannot attack.
trainer_two.switch_active_pokemon(1)
trainer_two.switch_active_pokemon(0)
